...
class MongoDbDriver(AbstractDbDriver):
    ...
    def validate_service(self) -> bool:
        ...
        if self.service["image"].split(":")[0] != IMAGE_NAME:
            raise DbValidationError(
                f"{self.service['$name']} service: A mongo database driver may only be used with '{IMAGE_NAME}' images."
            )
    ...
    def collect_additional_ports(self):
        return {"mysql": {"title": "MongoDB Port", "container": PORT, "host_start": PORT}}
